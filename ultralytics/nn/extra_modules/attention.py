...
class DSM_SpatialGate(nn.Module):
    def __init__(self, channel):
        ...
        # 修复：减少膨胀卷积的激进性
        self.dw1 = nn.Sequential(
            Conv(channel, channel, 3, s=1, d=1, g=channel, act=nn.GELU()),  # 减少膨胀
            nn.BatchNorm2d(channel),  # 添加BN稳定性
            Conv(channel, channel, 3, s=1, d=1, g=channel, act=nn.GELU())   # 减少膨胀
        )
        self.dw2 = nn.Sequential(
            Conv(channel, channel, kernel_size, g=channel, act=nn.GELU()),
            nn.BatchNorm2d(channel)  # 添加BN
        )
    ...
    def forward(self, x):
        ...
        # 限制数值范围
        dw1_out = torch.clamp(dw1_out, -10, 10)
        out = torch.clamp(out, -10, 10)
        dw2_out = torch.clamp(dw2_out, -10, 10)
        
        # 使用可学习权重进行稳定融合
        result = self.alpha * (dw1_out * out) + (1 - self.alpha) * dw2_out
...
class DSM_LocalAttention(nn.Module):
    def __init__(self, channel, p) -> None:
        ...
        # 修复：更稳定的参数初始化
        self.a = nn.Parameter(torch.ones(channel,1,1) * 0.01)  # 小的正值
        self.b = nn.Parameter(torch.ones(channel,1,1) * 0.99)  # 接近1但不是1
    ...
    def forward(self, x):        
        ...
        x_clamped = torch.clamp(x, -5, 5)
        
        # 避免三重乘法，使用更稳定的计算
        result = self.a * out * x_clamped + self.b * x_clamped
        return result
# class DSM_LocalAttention(nn.Module):
#     def __init__(self, channel, p) -> None:
#         super().__init__()
