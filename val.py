...
# print(torch.cuda.device_count())  # 应该返回 GPU 数量（>=1）
...
if __name__ == '__main__':
    model_path = '/root/ty/RTDETR-main/runs/train/ACD-DETR/weights/best.pt'
    model = RTDETR(model_path) # 选择训练好的权重路径
    # result = model.val(data='/root/ty/RTDETR-main/dataset/DOTA/DOTAv1.yaml', 
    # /root/ty/RTDETR-main/dataset/VisDrone/data.yaml
    result = model.val(data='/root/ty/RTDETR-main/dataset/VisDrone/data.yaml', 
                      split='val', # split可以选择train、val、test 根据自己的数据集情况来选择.
                      imgsz=640,
                      batch=32,
                      save_json=True, # if you need to cal coco metrice
                      project='runs/val', # 保存结果的路径
                      name='ACD-DETR', # exp
                      )  
    ...
    if model.task == 'detect': # 仅目标检测任务适用
        ...
        print(f"model_names 长度: {len(model_names)}")
        print(f"result.box.p 长度: {len(result.box.p)}")
        ...
        #                             ])
        # 使用较小的长度来避免越界
        max_idx = min(len(model_names), len(result.box.p))
        
        for idx in range(len(model_names)):
            cls_name = model_names[idx]
            if idx < len(result.box.p):
                # 有数据的类别
                model_metrice_table.add_row([
                    cls_name,
                    f"{result.box.p[idx]:.4f}",
